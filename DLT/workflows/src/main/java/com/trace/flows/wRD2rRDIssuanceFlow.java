...
@InitiatingFlow
@StartableByRPC
public class wRD2rRDIssuanceFlow extends FlowLogic<SignedTransaction> {

    private final Amount<Currency> amount;
    private final String peritelOwner;

    private final ProgressTracker.Step GENERATING_TRANSACTION = new ProgressTracker.Step("Generating transaction based on new wRD to rRD issuance.");
    ...
    public wRD2rRDIssuanceFlow(Amount<Currency> amount, String peritelOwner) {
        this.amount = amount;
        this.peritelOwner = peritelOwner;
    }
    ...
    public SignedTransaction call() throws FlowException {
        ...
        List<StateAndRef<wRDAccountState>> wholesalerStates = getServiceHub().getVaultService().queryBy(wRDAccountState.class, generalCriteria).getStates()
        ...
        List<StateAndRef<rRDAccountState>> peritelStates = getServiceHub().getVaultService().queryBy(rRDAccountState.class, generalCriteria).getStates()
                .stream().filter(stateAndRef -> stateAndRef.getState().getData().getOwner().equals(peritelOwner)).collect(java.util.stream.Collectors.toList());

        if (wholesalerStates.isEmpty() || peritelStates.isEmpty()) {
        ...
        final TransactionBuilder txBuilder = new TransactionBuilder(notary)
                ...
                .addCommand(new rRDContract.Commands.wRD2rRDIssuanceCommand(), getOurIdentity().getOwningKey());

        if (observer != null) {
            txBuilder.addCommand(new rRDContract.Commands.wRD2rRDIssuanceCommand(), observer.getOwningKey());
        }
