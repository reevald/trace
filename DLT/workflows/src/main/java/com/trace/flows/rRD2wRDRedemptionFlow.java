...
@InitiatingFlow
@StartableByRPC
public class rRD2wRDRedemptionFlow extends FlowLogic<SignedTransaction> {

    private final Amount<Currency> amount;
    private final UniqueIdentifier sourceWalletId;
    private final UniqueIdentifier targetWalletId;

    private final ProgressTracker.Step GENERATING_TRANSACTION = new ProgressTracker.Step("Generating transaction based on new rRD to wRD redemption.");
    ...
    public rRD2wRDRedemptionFlow(Amount<Currency> amount, UniqueIdentifier sourceWalletId, UniqueIdentifier targetWalletId) {
        this.amount = amount;
        this.sourceWalletId = sourceWalletId;
        this.targetWalletId = targetWalletId;
    }

    public rRD2wRDRedemptionFlow(Amount<Currency> amount, String peritelOwner) {
        this.amount = amount;
    ...
    public SignedTransaction call() throws FlowException {
        ...
        if (targetWalletId != null) {
            List<java.util.UUID> targetWalletIds = java.util.Collections.singletonList(targetWalletId.getId());
            QueryCriteria linearIdCriteria = new QueryCriteria.LinearStateQueryCriteria(null, targetWalletIds);
            QueryCriteria combinedCriteria = generalCriteria.and(linearIdCriteria);
        ...
        if (peritelStates.isEmpty() || wholesalerStates.isEmpty()) {
        ...
        wRDAccountState wholesalerOutputState = new wRDAccountState(
                ...
                wholesalerInputState.getState().getData().getTokenBalance().plus(amount)
        );
        ...
        final TransactionBuilder txBuilder = new TransactionBuilder(notary)
                ...
                .addCommand(new rRDContract.Commands.rRD2wRDRedemptionCommand(), getOurIdentity().getOwningKey());

        if (observer != null) {
            txBuilder.addCommand(new rRDContract.Commands.rRD2wRDRedemptionCommand(), observer.getOwningKey());
        }
