...
public class wRDRedemptionFlow extends FlowLogic<SignedTransaction> {
    ...
    public wRDRedemptionFlow(UniqueIdentifier sourceWalletId, UniqueIdentifier receiverWalletId,
    ...
    public SignedTransaction call() throws FlowException {
        ...
        if (sourceWholesaler.getName().getOrganisation().equals("KDR")) {
            throw new FlowException("Source party must be non-KDR.");
        }

        // 1.3 KDR party (receiver)
        final Party kdr = getServiceHub().getNetworkMapCache().getPeerByLegalName(CordaX500Name.parse("O=KDR," +
                "L=Jakarta,C=ID"));
        if (kdr == null) {
            throw new FlowException("KDR party not found.");
        }
        ...
        for (StateAndRef<wRDAccountState> stateAndRef : stateAndRefOfSourceAndReceiverWalletStatesInSourceParty) {
            ...
            if (stateAndRef.getState().getData().getWalletId().equals(receiverWalletId)) {
                if (!stateAndRef.getState().getData().getOwner().equals(kdr)) {
                    throw new IllegalArgumentException("Receiver wallet state must be owned by receiver party (KDR).");
                }
        ...
        FlowSession session = initiateFlow(kdr);
        ...
        wRDAccountState outputSourceWalletState =
                stateAndRefSourceWalletStateDetermined.getState().getData()
                        .withNewBalanceAndIssuer(remainingBalanceResourceWallet, kdr);
        ...
        // 4.3 Add command related with wRDRedemptionFlow
        ...
        txBuilder.addCommand(new wRDContract.Commands.wRDRedemptionCommand(), mergedListOfRequiredSigners);
        ...
        for (AbstractParty participant: stateAndRefSourceWalletStateDetermined.getState().getData().getParticipants()) {
            ...
            if (!partyToInitiateFlow.getOwningKey().equals(getOurIdentity().getOwningKey())
                    && !partyToInitiateFlow.getOwningKey().equals(kdr.getOwningKey())) {
                FlowSession sessionSignOnly = initiateFlow(partyToInitiateFlow);
                // Simple approach to bypass blocking receive syncStatRequest in responder flow
                sessionSignOnly.send("SIGN_ONLY");
                sessions.add(sessionSignOnly);
            }
        }
